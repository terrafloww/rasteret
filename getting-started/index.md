...
[marimo](https://marimo.io) manages dependencies inline, no kernel registration needed. Just `uv pip install marimo` alongside Rasteret and run `marimo edit notebook.py`.

VS Code auto-detects virtual environments. Open a `.ipynb` file, click the kernel picker (top-right), and select the Python interpreter from your Rasteret venv.
...
Pick any dataset ID and pass it to `build()` in the next section. For data not in the catalog, use `build_from_stac()` or `build_from_table()`, or [add it to the catalog](https://terrafloww.github.io/rasteret/how-to/dataset-catalog/#add-your-own-catalog-entries-advanced) so the whole community benefits.
...
Rasteret searched the STAC catalog, parsed the COG tile-layout headers for every matching scene, and cached everything in a local Parquet index (in `~/rasteret_workspace/` by default). The next call with the same parameters reuses the cached index instantly; no network requests, no header parsing. This is the core idea: **build once, read many times**.

Collection vs catalog

A Collection contains only the scenes you indexed (your bbox, date range, filters). Use `compare_to_catalog()` to see what you built vs what the source offers:
...
This example uses Sentinel-2 from Earth Search, which is public; no credentials needed. For requester-pays or authenticated sources, see [Custom Cloud Provider](https://terrafloww.github.io/rasteret/how-to/custom-cloud-provider/index.md).
...
Everything after `to_torchgeo_dataset()` is standard TorchGeo - your samplers, transforms, and training loop stay the same.
...
`geometries` also accepts Arrow arrays (from GeoParquet), Shapely objects, or raw WKB. See [Build from Parquet](https://terrafloww.github.io/rasteret/how-to/build-from-parquet/index.md) for the Arrow-native path.
...
Use this when you want direct NumPy output for custom ML/data pipelines without xarray coordinates.
...
Use this when you need tabular point features (e.g. millions of points, joins with metadata tables, model feature stores).

`samples` is a `pyarrow.Table`, so you can process it directly with Arrow-native tools. (Shapely points are also supported for quick scripts.)

That's it for the basics. Two calls: `build()` to index, then read pixels.

Data types

Rasteret preserves the native COG dtype. Sentinel-2 data returns as `uint16`, not `float32`. If you need float for computation (e.g., NDVI), cast explicitly: `ds.B04.astype("float32")`.
...
| I want to...                                                              | Go to                                                                                                          |
| ------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| Follow a hands-on notebook                                                | [Tutorials](https://terrafloww.github.io/rasteret/tutorials/index.md)                                          |
| Build from existing Parquet (Source Cooperative, STAC GeoParquet, custom) | [Build from Parquet](https://terrafloww.github.io/rasteret/how-to/build-from-parquet/index.md)                 |
| Sample values at many points (Arrow-first)                                | [Point Sampling and Masking](https://terrafloww.github.io/rasteret/how-to/point-sampling-and-masking/index.md) |
| Enrich a collection with AOIs and labels                                  | [Enriched Parquet Workflows](https://terrafloww.github.io/rasteret/how-to/enriched-parquet-workflows/index.md) |
| Use train/val/test splits and labels                                      | [ML Training with Splits](https://terrafloww.github.io/rasteret/how-to/ml-training-splits/index.md)            |
| Manage cached collections (build/import/list/info/delete)                 | [Collection Management](https://terrafloww.github.io/rasteret/how-to/collection-management/index.md)           |
| Browse the dataset catalog and reuse local Collections                    | [Dataset Catalog](https://terrafloww.github.io/rasteret/how-to/dataset-catalog/index.md)                       |
| Read authenticated/requester-pays data (Landsat, PC, Earthdata)           | [Custom Cloud Provider](https://terrafloww.github.io/rasteret/how-to/custom-cloud-provider/index.md)           |
| Understand how Rasteret works (Explanation)                               | [Explanation](https://terrafloww.github.io/rasteret/explanation/index.md)                                      |
| See how Rasteret fits with TorchGeo/xarray/rasterio                       | [Ecosystem Comparison](https://terrafloww.github.io/rasteret/explanation/interop/index.md)                     |
| See the full API                                                          | [API Reference](https://terrafloww.github.io/rasteret/reference/index.md)                                      |
