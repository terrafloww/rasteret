...
**The pixels** stay where they already are: in the original cloud or local COGs. Rasteret only reads pixel byte ranges when you call methods such as `get_numpy()`, `get_xarray()`, `get_gdf()`, `sample_points()`, or `read_window()` (used internally by `RasteretDataset` in TorchGeo).
...
1. **Read**: Choose `get_numpy()`, `get_xarray()`, `get_gdf()`, `sample_points()`, or `RasteretDataset` from TorchGeo.
