...
`RasteretDataset` in `torchgeo.datasets` is a `GeoDataset` backed by a Rasteret `Collection`. TorchGeo samplers, transforms, composition, and dataloaders stay standard; pixel reads are served by Rasteret's async byte-range engine.
...
Pre-filter with `collection.subset(...)` or `collection.where(...)` before constructing the dataset if you need a geographic or attribute subset.
