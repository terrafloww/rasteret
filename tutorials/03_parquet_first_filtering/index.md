# Parquet Filtering[¶](#parquet-filtering)

Rasteret's core loop: **cache once, filter anywhere**.

This notebook covers:

1. Building a Collection with `build()` (creates a cached Parquet index)
1. Exporting a snapshot to a new Parquet path (optional) and reloading
1. Discovering cached collections
1. Filtering with `subset()` (cloud cover, date range, bbox)
1. Loading from an existing Parquet file with `build_from_table()`
1. Arrow expression pushdown with `filter_expr`
...
workspace = Path.home() / "rasteret_workspace" collection = rasteret.build( "earthsearch/sentinel-2-l2a", name="bangalore-s2-demo", bbox=(77.5, 12.9, 77.7, 13.1), date_range=("2024-01-01", "2024-06-30"), workspace_dir=workspace, ) print(f"Scenes: {collection.dataset.count_rows()}")
...
Next: [Custom Cloud and Bands](https://terrafloww.github.io/rasteret/tutorials/04_custom_cloud_and_bands/index.md)
