# Building from Parquet[¶](#building-from-parquet)

Tutorials 01-02 build Collections from a STAC API via `build()`. But Rasteret works with **any Parquet file that has GeoTIFF URLs**; no STAC API needed.

`build_from_table()` reads Parquet directly from S3, GCS, or local paths. It validates the schema, derives bounding boxes, and produces a standard Collection backed by Arrow.

This notebook uses the [Maxar Open Data](https://www.maxar.com/open-data) catalog from [Source Cooperative](https://source.coop/maxar/maxar-opendata): 14,979 sub-meter satellite scenes across 23 disaster events, published as STAC GeoParquet with fully public COGs.
...
Next: [Parquet Filtering](https://terrafloww.github.io/rasteret/tutorials/03_parquet_first_filtering/index.md)
