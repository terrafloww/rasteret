...
`RasteretDataset(collection, bands, time_series=True)` is a standard `GeoDataset`, compatible with the same samplers, DataLoader, and `stack_samples` collate function as Path A. Each sample returns `[T, C, H, W]` with all timesteps stacked.
...
t0 = time.perf_counter()
rasteret_dataset = RasteretDataset(
...
# Pre-filter by bbox before building the dataset if needed;
# the collection was already built from the covering_bbox above.
...
t0 = time.perf_counter()
rasteret_multi = RasteretDataset(
    collection=collection_multi,
    bands=[BAND],
    time_series=True,
)
...
- **Multi-AOI filtering**: To restrict sampling to specific AOIs, pre-filter the collection before passing it to `RasteretDataset`. For a covering bbox use `collection.subset(bbox=...)`. After that, TorchGeo's `roi=` argument on the sampler restricts chip draws to any polygon in the dataset's CRS.
...
- **AOI-only sampling**: To restrict chips to a polygon (e.g. a county boundary), pass `roi=<AOI polygon in dataset CRS>` to `GridGeoSampler` / `RandomGeoSampler`. The `roi` is separate from the dataset index and does not require rebuilding the collection.
...
# crs=CRS.from_epsg(TARGET_CRS) keeps scenes from BOTH UTM zones and
# reprojects at read time via rasterio.warp.reproject() after concurrent fetch.
...
t0 = time.perf_counter()
rasteret_crs = RasteretDataset(
    collection=collection_crs,
    bands=[BAND],
    time_series=True,
    crs=ProjCRS.from_epsg(TARGET_CRS),
)
t_index_crs_b = time.perf_counter() - t0

print(f"CRS (target): EPSG:{TARGET_CRS}")
print(f"Scenes in index: {len(rasteret_crs.index)}")
