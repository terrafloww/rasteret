...
workspace = (Path.cwd() / ".cache" / "notebooks" / "01_quickstart").resolve()
shutil.rmtree(workspace, ignore_errors=True)
workspace.mkdir(parents=True, exist_ok=True)

collection = rasteret.build(
    "earthsearch/sentinel-2-l2a",
    name="bangalore-quickstart",
    bbox=aoi.bounds,
...
import shutil workspace = (Path.cwd() / ".cache" / "notebooks" / "01_quickstart").resolve() shutil.rmtree(workspace, ignore_errors=True) workspace.mkdir(parents=True, exist_ok=True) collection = rasteret.build( "earthsearch/sentinel-2-l2a", name="bangalore-quickstart", bbox=aoi.bounds, date_range=("2024-01-01", "2024-01-31"), workspace_dir=workspace, ) print(f"Collection: {collection.name}") print(f"Scenes: {collection.dataset.count_rows()}") print(f"Columns: {collection.dataset.schema.names[:8]}...")
...
gdf = collection.get_gdf( geometries=[aoi], bands=["B04", "B08"], cloud_cover_lt=20, date_range=("2024-01-10", "2024-01-30"), ) gdf.head()
...
Next: [TorchGeo Integration](https://terrafloww.github.io/rasteret/tutorials/02_using_rasteret_with_torchgeo/index.md)
