...
from torchgeo.datasets import RasteretDataset dataset = RasteretDataset( collection=collection, bands=["B02", "B03", "B04", "B08"], ) print(f"Type: {type(dataset).__mro__[:3]}") print(f"Bounds: {dataset.bounds}") print(f"CRS: {dataset.crs}")

```
Type: (<class 'torchgeo.datasets.rasteret.RasteretDataset'>, <class 'torchgeo.datasets.geo.GeoDataset'>, <class 'torch.utils.data.dataset.Dataset'>)
Bounds: (slice(np.float64(699960.9999999993), np.float64(809758.9999999998), 10.0), slice(np.float64(1390200.9999999998), np.float64(1499999.0000000002), 10.0), slice(Timestamp('2024-01-03 05:25:20.281000'), Timestamp('2024-01-28 05:25:14.668000'), 1))
CRS: EPSG:32643
```
...
```
/tmp/ipykernel_757577/877709070.py:5: DeprecationWarning: Use torchgeo.samplers.RandomPatchSampler instead
  sampler = RandomGeoSampler(dataset, size=256, length=16)
```

```
image shape:     torch.Size([4, 4, 256, 256])
bounds shape:    torch.Size([4, 9])
transform shape: torch.Size([4, 9])
```
