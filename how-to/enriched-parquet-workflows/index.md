...
1. **You enrich** it with experiment-specific columns
1. **Arrow tools filter** the enriched Parquet (DuckDB, PyArrow, GeoPandas)
1. **Rasteret fetches** COG pixels for the filtered subset
...
Use this pattern when you want Major TOM-like metadata ergonomics but keep image bytes in source Sentinel-2 COGs. Parquet stays the control plane; Rasteret reads pixels directly from remote COGs.
...
1. Uses `majortom.grid.Grid` on scene centroids to derive `major_tom_grid_cell`
1. Uses STAC `s2:product_uri` (without `.SAFE`) as `major_tom_product_id`
1. Adds deterministic split labels from `grid_cell`
1. Exports a shareable scene-level Collection that downstream tools can query

### Retrieve pixels from Arrow geometry

Retrieval is scene-batched: patches are grouped by `major_tom_product_id`, and each scene's patches are fetched in one `get_numpy()` call with an Arrow WKB geometry array:
...
Baseline method: Hugging Face `datasets.load_dataset(..., streaming=True, filters=...)` (PyArrow-backed predicate pushdown) for keyed retrieval. This is stronger than the streaming-iterator style commonly used in Major TOM exploration notebooks without keyed filters.

For a larger, benchmark-focused scene cache, see `examples/major_tom_benchmark/01_build_collection_sharded.py`.
