...
- [Build from Parquet and Arrow Tables](https://terrafloww.github.io/rasteret/how-to/build-from-tables/index.md) for external Parquet, GeoParquet, DuckDB, Polars, or Arrow record tables.
- [Dataset Catalog](https://terrafloww.github.io/rasteret/how-to/dataset-catalog/index.md) for built-in dataset IDs and local catalog registration.
- [Custom Cloud Provider](https://terrafloww.github.io/rasteret/how-to/custom-cloud-provider/index.md) for requester-pays, authenticated buckets, URL rewriting, and custom backends.
...
You can pass `workspace_dir=...` in Python or `--workspace-dir ...` in the CLI when you want a different location.
...
`collections import` materializes a local collection from the record table. If your source table still needs COG header enrichment for pixel reads, use the Python `build_from_table(..., enrich_cog=True)` path instead.
...
Use `--json` on `build`, `import`, `list`, and `info` when scripting. Use `--yes` with `delete` to skip the confirmation prompt.
...
When `name` is provided, Rasteret can cache the built collection in the default workspace with the given name as the folder name.

Pass `force=True` when you want to rebuild an existing cache.

### Filter At Build Time

Both `rasteret.build(...)` and `rasteret.build_from_stac(...)` accept a `query=` dict that gets forwarded to the STAC API's search endpoint. Use this when you know upfront which scenes to keep — it's cheaper to filter at the STAC level than to build a wide index and subset it later.
...
The `query` keys are STAC item properties; the values use the STAC API query extension's operators (`lt`, `lte`, `gt`, `gte`, `eq`, `neq`, `in`). For post-build filtering on an already-indexed collection, see [Filter Metadata Before Reading](#filter-metadata-before-reading) below.
...
This is useful when you built a date or AOI subset and want to see how it relates to the full catalog entry. If there is no matching catalog descriptor, use `collection.describe()` instead.
...
Each item includes fields such as `name`, `kind`, `data_source`, `date_range`, `size`, and `created`.
...
For labels, splits, AOI joins, point tables, and other business metadata, see [Bring Your Own AOIs, Points, And Metadata](https://terrafloww.github.io/rasteret/how-to/enriched-collection-workflows/index.md).
