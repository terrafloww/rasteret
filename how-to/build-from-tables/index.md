...
For least friction across `build_from_table(...)`, `subset(date_range=...)`, and downstream reads:

- keep `datetime` as a real Arrow/Pandas datetime dtype, not `object` strings
- prefer UTC timestamps
- avoid raw ISO strings such as `"2024-01-01T00:00:00Z"` unless you convert them first
...
If your table uses a year column (for example `2023`, `2024`), map it with `column_map={"year": "datetime"}`. Rasteret normalizes integer years to timestamps at January 1st of each year.
