...
`RasteretDataset` in `torchgeo.datasets` provides **pipeline-level interop**:
samplers, DataLoader, and training code stay in TorchGeo, while Rasteret
provides fast pixel I/O via `read_window()` underneath.
